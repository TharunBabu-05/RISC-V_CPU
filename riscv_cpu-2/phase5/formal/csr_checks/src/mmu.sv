...
module mmu #(
    ...
    logic [8:0] satp_asid;  // ASID from satp[30:22]
    logic mmu_en;

    assign satp_mode = satp[31];
    assign satp_ppn  = satp[PPN_BITS-1:0];
    assign satp_asid = satp[30:22];
    // MMU enabled: Sv32 mode active (M-mode exempt from translation on real HW, but enabled for testing)
    ...
    logic [8:0]          itlb_asid  [0:TLB_ENTRIES-1];  // ASID tag
    ...
    logic [8:0]          dtlb_asid  [0:TLB_ENTRIES-1];  // ASID tag
    ...
    always_comb begin
        ...
        for (int i = 0; i < TLB_ENTRIES; i = i + 1) begin
            // TLB hit: valid, VPN match, and ASID match
            if (itlb_valid[i] && itlb_vpn[i] == inst_vpn && itlb_asid[i] == satp_asid) begin
                itlb_hit = 1'b1;
                itlb_idx = i;
            end
        end
    ...
    always_comb begin
        ...
        for (int i = 0; i < TLB_ENTRIES; i = i + 1) begin
            // TLB hit: valid, VPN match, and ASID match
            if (dtlb_valid[i] && dtlb_vpn[i] == data_vpn && dtlb_asid[i] == satp_asid) begin
                dtlb_hit = 1'b1;
                dtlb_idx = i;
            end
        end
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (sfence || satp != satp_last) begin
                satp_last <= satp;
                state <= S_IDLE;
                itlb_wr_ptr <= 0;
                dtlb_wr_ptr <= 0;
                ...
                case (state)
                    ...
                    S_L1: begin
                        ...
                        if (!pte_v || (pte_r && !pte_w && !pte_x)) begin
                            ...
                            if (!pte_ok) begin
                                ...
                                dtlb_asid[dtlb_wr_ptr]  <= satp_asid;
                    ...
                    S_L0: begin
                        ...
                        if (!pte_v || (!pte_r && pte_w) || (!pte_r && !pte_x)) begin
                            fault_pending <= 1'b1;
                            fault_is_inst <= req_is_inst;
                            state <= S_IDLE;
                        end else begin
